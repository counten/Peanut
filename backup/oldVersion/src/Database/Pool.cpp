...
MYSQL *Pool::createOneConnect()
{
    ...
    if (conn != NULL)
    {
        ...
        {
            Log::logError("" + mysql_errno(conn));
            return NULL;
        }
    }
    else
    {
        Log::logError("init failed");
        return NULL;
    }
}
...
MYSQL *Pool::getOneConnect()
{
    ...
    if (!isEmpty()) {
        ...
        else
        {
            ...
            {
                Log::logError("Connections is too much!" );
            }
        }
    }
    else
 ...
 * @return queryResult
 * */
QueryResult Pool::executeSql(const char *sql)
{
    Log::logInfo(sql);
    ...
    if (conn)
    {
        if (mysql_query(conn, sql) == 0)
        {
            ...
            Log::logSuccess("Query OK! ");
        }
        else
        {
            Log::logError(mysql_error(conn));
        }
        close(conn);
    }
    else
